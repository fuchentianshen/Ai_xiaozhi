...
void audio_encoder_task(void *arg)
{
    ...
    while (encoder->is_running)
    {
        // 从输入缓冲区中取出数据
        size_t size_read = 0;
        void *buf_read = xRingbufferReceiveUpTo(encoder->input_buffer, &size_read, pdMS_TO_TICKS(100), in_frame_size);
        if (!buf_read)
        {
            continue;
        }
...
audio_encoder_t *audio_encoder_create(RingbufHandle_t input_buffer, RingbufHandle_t output_buffer, int sample_rate, int bit_per_sample, int channels)
{
    // 申请内存
    ...
    esp_audio_enc_config_t enc_config = {
        ...
        .cfg = &opus_config,
        .cfg_sz = sizeof(opus_config),
    };
    ESP_ERROR_CHECK(esp_audio_enc_open(&enc_config, &encoder->enc));
    return encoder;
}
...
void audio_encoder_start(audio_encoder_t *encoder)
{
    ...
    xTaskCreatePinnedToCoreWithCaps(audio_encoder_task,
                                    "encoder_task",
                                    AUDIO_ENCODER_TASK_STACK_SIZE,
                                    encoder,
                                    AUDIO_ENCODER_TASK_PRIORITY,
                                    NULL,
                                    AUDIO_ENCODER_TASK_CORE_ID,
                                    MALLOC_CAP_SPIRAM);
}
