...
protocol_t *protocol_create(const char *url, const char *token); // 创建一个协议实例
void protocol_destroy(protocol_t *protocol);                     // 销毁一个协议实例

void protocol_connect(protocol_t *protocol);      // 连接
void protocol_disconnect(protocol_t *protocol);   // 断开
bool protocol_is_connected(protocol_t *protocol); // 检查连接状态
