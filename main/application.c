...
typedef struct
{
    app_state_t state;
    protocol_t *protocol;
    audio_processer_t *audio_processer;
                                          ...
                                          void *event_data)
{
    ...
    case PROTOCOL_EVENT_DISCONNECTED: // event_data为NULL
        audio_processer_set_vad_state(self->audio_processer, false);
    ...
    case PROTOCOL_EVENT_TTS_STOP: // event_data为NULL
        ...
        {
            application_set_state(self, APP_STATE_WAKEUP);
            audio_processer_set_vad_state(self->audio_processer, true);
        }
        break;
    case PROTOCOL_EVENT_AUDIO: // event_data为binary_data_t*
        if (self->state == APP_STATE_SPEAKING)
        {
            ...
            audio_processer_write(self->audio_processer, audio_data->data, audio_data->size);
        }
        break;
...
static void application_audio_processer_callback(void *event_handler_arg,
                                                 esp_event_base_t event_base,
                                                 int32_t event_id,
                                                 void *event_data)
{
...
static void application_upload_task(void *arg)
{
    ...
    while (1)
    {
        size_t size = audio_processer_read(self->audio_processer, buffer, sizeof(buffer));
        if (self->state == APP_STATE_LISTENING)
        {
...
void application_init(void)
{
    ...
    s_app.audio_processer = audio_processer_create();
    audio_processer_register_callback(s_app.audio_processer, AUDIO_SR_EVENT_SILIENCE, application_audio_processer_callback, &s_app);
    audio_processer_register_callback(s_app.audio_processer, AUDIO_SR_EVENT_SPEECH, application_audio_processer_callback, &s_app);
    audio_processer_register_callback(s_app.audio_processer, AUDIO_SR_EVENT_WAKEUP, application_audio_processer_callback, &s_app);

    audio_processer_start(s_app.audio_processer);

    // 启动后台上传任务
    xTaskCreatePinnedToCoreWithCaps(application_upload_task, "upload_task", 4096, &s_app, 6, NULL, 0, MALLOC_CAP_SPIRAM);

    // 初始情况下关闭vad
    audio_processer_set_vad_state(s_app.audio_processer, false);
